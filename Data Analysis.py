...
print(per_table)
print(per_table['RevolvingUtilizationOfUnsecuredLines','<lambda_0>'])
per_table.head()
per_table['SeriousDlqin2yrs','percent']=per_table['SeriousDlqin2yrs','sum']/per_table['RevolvingUtilizationOfUnsecuredLines','len']*100
per_table=per_table.rename(columns={'<lambda>':'percent'})
per_table=per_table.reindex((per_table.columns[1],per_table.columns[0],per_table.columns[2],per_table.columns[3]),axis=1)
print(per_table)
print(per_table)
...
def get_frequency(df,col_x,col_y, cut_points, labels,isright=True):
    ...
    if(per_table.columns[0][0]!=col_x): #假如col_x不在第一列，说明是在第2、3列，就把它们往前挪
        per_table=per_table.reindex((per_table.columns[1],per_table.columns[2],per_table.columns[0]),axis=1)
